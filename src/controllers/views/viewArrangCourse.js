...
const debug = require('debug')('app:controllers:views:viewArrangCourse');
const async = require('async');
const _ = require('underscore');
const Boom = require('boom');
// const moment = require('moment');

const ViewArrangCourse = require('../../models/views/viewArrangCourse');
...
const ViewStudentMethods = {
    ...
    count(request, reply) {
        ...
        async.waterfall([ // 查询班级
            (cb) => {
                ViewArrangCourse.findAndCountAll(queryObj).then(function (classModel) {
                    reply(classModel)
                }).catch(function (err) {
                    // console.log('err', err);
